...
void Vector<string>::sort() {
	//[TODO]
	Vector<string> copy_sort(*this);
	for (int i = 0; i < copy_sort.size(); ++i)
	{
	 	string str = copy_sort[i];
	 	int len = str.length();
	...
	for(int i = 1; i < copy_sort.size(); i++)  
    {  
     	for(int j = i; j > 0; j--)  
        {  
          	if(copy_sort[j] < copy_sort[j-1])
          	{  
	            string tmp = copy_sort[j];  
    			copy_sort[j] = copy_sort[j-1];  
    			copy_sort[j-1] = tmp;  
            }
            else  
                break;  
