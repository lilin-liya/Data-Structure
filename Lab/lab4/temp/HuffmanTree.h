...
void HuffmanTree::GetoriginChar(HuffmanNode* node,string str,int deep)
{
  //cout<<str<<endl;
  if (deep == str.length())
  {
  ...
  if (node->left == NULL && node->right == NULL)
  {
    ...
    chars_get.push_back(node->ch);
    //deep++;
    cout<<(char)node->ch<<endl;
    node = root;
    return;
  }
  if (str[deep] == '0' && node->left != NULL)
  {
    cout<<"deep: "<<deep<<endl;
    GetoriginChar(node->left,str,deep+1);
  }
  if (str[deep] == '1' && node->right != NULL)
  {
    cout<<"deep: "<<deep<<endl;
    GetoriginChar(node->right,str,deep+1);
  }
}
